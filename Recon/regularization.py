...
def Ls_CNNMHnor(im,y,paramImg,paramReg):
    input_im = tf.convert_to_tensor(im.reshape(paramImg.tensor_shape))
    output = paramImg.model(input_im)
    f = 0
    for i_e in range(len(paramImg.energy_tgt)):
        f_temp = 0.5*paramReg.gamma[i_e]*tf.square(paramImg.mu_max[i_e]*output[i_e]-y[i_e])
        f_temp = f_temp.numpy().reshape(paramImg.img_shape)
        f = f + np.sum(f_temp)
        #print(type(output[i_e]))
    g = np.zeros(paramImg.img_shape)
    with tf.GradientTape(persistent=True) as tape:
        tape.watch(input_im)
        outputg = paramImg.model(input_im)
        for i_e in range(len(paramImg.energy_tgt)):
            ff = 0.5*paramReg.gamma[i_e]*tf.square(paramImg.mu_max[i_e]*outputg[i_e] - y[i_e])
            g_temp = tape.gradient(ff, input_im)
            g = g + g_temp.numpy().reshape(paramImg.img_shape)
    g = g.flatten(order='F')
...
def fullcostsumMHnor_vec(im_vec,y,paramImg,paramReg):
    im = im_vec.reshape(paramImg.img_shape, order='F')
    f, g = Ls_CNNMHnor(im, y, paramImg, paramReg)
    fr, gr = imagePrior(im, paramReg.paramRecon)
...
def min_z_lbfgs_MH_normalized(ct_input, ct_tgt, paramImg, paramReg):
    z = ct_input
    z_vecinit = z.flatten(order='F')

    bnds = [(0, 1)] * z_vecinit.size

    cost_1var = lambda im_vec: fullcostsumMHnor_vec(im_vec, ct_tgt, paramImg, paramReg)
